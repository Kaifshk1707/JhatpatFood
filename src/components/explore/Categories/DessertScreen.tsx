...
const DessertScreen = () => {
  ...
  const fetchDessertData = useCallback(async () => {
    setLoading(true);
    try {
      const DessertCollection = await firestore().collection("dessert").get();
      const dessertFoodList = DessertCollection.docs.map((doc) => {
        const data = doc.data();
        return {
          ...data,
          id: doc.id,
  ...
  const renderCard = (item: FoodItem) => (
    ...
    >
      ...
      >
        ...
        >
          <Ionicons name={"heart-outline"} size={RFValue(18)} color="#AB47BC" />
        </TouchableOpacity>
      </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#FAFAFA" }}>
      ...
      >
        <Ionicons name="arrow-back" size={RFValue(22)} color="#AB47BC" />
      </TouchableOpacity>
